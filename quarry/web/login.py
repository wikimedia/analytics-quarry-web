...
user_agent = (
    "Quarry %s (https://wikitech.wikimedia.org/wiki/Nova_Resource:Quarry) Python-requests/%s"
...
def oauth_callback():
    ...
    try:
        access_token = handshaker.complete(
            session["request_token"], request.query_string
        )
    except (OAuthException, KeyError) as e:
        exception(e)
        flash(
            "Fatal error occured while login (%s). Please try again after cleaning the cookies in your browser."
            % str(e),
            "danger",
        )
