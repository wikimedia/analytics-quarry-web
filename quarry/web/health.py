...
def fetch_queries_count_last_minutes(table, column, minutes):
    return (
        g.conn.session.query(table)
        .filter(column >= text("NOW() - INTERVAL %d MINUTE" % minutes))
        .count()
    )
...
def health_summary(minutes):
    ...
    statuses = (
        g.conn.session.query(QueryRun.status)
        .filter(
    ...
    for row in statuses:
        ...
        resp_dict_query_statuses[
            QueryRun.STATUS_MESSAGES[query_run_status]
        ] += 1
